...
char retarrow[8] = {0x10,0x10,0x14,0x12,0x1F,0x02,0x04,0x00};
// char clock[8] = {0x00,0x0E,0x15,0x17,0x11,0x0E,0x00,0x00};
...
char interrupts_getkey(bool IRQ_flag){
    ...
    if (IRQ_flag){
        ...
        keyChange = false;
        if (index != 16) pad_chart = keymap[index];
        else pad_chart = keymap[16];
    }
    return pad_chart;
}
...
void SetTimer(TIME *p, unsigned char *Flag){
    ...
    int buffer=0;
    delay(150);
    pad_chart = interrupts_getkey(keyChange);
...
void ActualActivation(Menu *menu, int lcd_select){
    TIME *p;
    switch (menu->MenuID){
    ...
    case OnOffRelay:
        if(lcd_select == 3) relays.relayall ^= 0xFF;
        break;

    case TimerRelay:
        if(lcd_select == 3) for(int i=0; i<8; i++){ TimerFlag[i] = 0; };
        break;
...
int lcd_select=1, new_menu = 0;
void MenuDisplay(Menu *menu, int lcd_select){
    ...
    // lcd.setCursor(0,lcd_select); lcd.print(">");
    lcd.setCursor(0,lcd_select); lcd.write(0);
} 
